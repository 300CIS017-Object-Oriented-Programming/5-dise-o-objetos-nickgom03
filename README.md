...
Este diagrama representa cómo un `Usuario` puede crear múltiples `Publicaciones`, estableciendo una relación  entre ambas entidades. Para modelar mejor esta relación, se incluye el arreglo - `Publicacion [] publicaciones` dentro de la clase `Usuario`, lo que indica que cada usuario tiene un conjunto de publicaciones asociadas. 
...
📌 **Ejemplo 1:** Imagina que estás programando un juego de carreras. La clase `Carro` define atributos como `marca`, `color` y `velocidadMáxima`, pero cada carro en la pista (un objeto) tendrá valores distintos para estos atributos. Un carro rojo con velocidad máxima de 200 km/h y otro azul con 220 km/h son diferentes objetos de la misma clase `Carro`.
